...
import 'package:sosang/model/prefStorage.dart';
...
class GV {
  static void printLongString(String longString) {
    ...
    for (var element in tr1.frames) {
      if (element.location.contains(PACKAGENAME) &&
          !element.member!.contains("printLongString")) newFr1.add(element);
    }
    print('========');
  ...
  static Future<void> init(String logel) async {
    ...
    switch (logel.toUpperCase()) {
      case 'ERROR':
        logger = Logger(
            printer: PrettyPrinter(printTime: false, lineLength: 40),
            level: Level.error);
        break;
      case 'DEBUG':
        logger = Logger(
            printer: PrettyPrinter(printTime: false, lineLength: 4),
            level: Level.debug);
        break;
      case 'INFO':
        logger = Logger(
...
//===========================================================================
  ...
  static set eventBus(EventBus value) {
    locator.registerLazySingleton<EventBus>(() => value);
  }
}
